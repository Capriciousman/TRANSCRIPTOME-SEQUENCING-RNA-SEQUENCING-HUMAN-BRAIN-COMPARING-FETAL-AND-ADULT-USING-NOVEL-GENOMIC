...
c = read.table(file.choose(),header = TRUE)
d = read.table(file.choose(),header = TRUE)
e = read.table(file.choose(),header = TRUE)
f = read.table(file.choose(),header = TRUE)

merge_countfile = a
merge_countfile = cbind(a,b$alignments)
merge_countfile = cbind(merge_countfile,c$alignments)
merge_countfile = cbind(merge_countfile,d$alignments)
merge_countfile = cbind(merge_countfile,e$alignments)
merge_countfile = cbind(merge_countfile,f$alignments)
head(merge_countfile)
merge_countfile = as.data.frame(merge_countfile)
colnames(merge_countfile)
colnames(merge_countfile) = paste(c("geneid","sample1","sample2","sample3","sample4","sample5","sample6"))
write.table(merge_countfile,file = "mergecountfile.txt",sep = "\t",col.names = TRUE) 



```
#Deseq for RNA differential expression analysis
```{r}

merge_countfile
##Remove the first column and make another dataframe
new_merge_countfile = merge_countfile[,2:ncol(merge_countfile)]
head(new_merge_countfile)
boxplot(new_merge_countfile,xlab="x axis")
plot(new_merge_countfile)
pca = prcomp(t(new_merge_countfile))
...
design = data.frame(row.names = c("geneid","sample1","sample2","sample3","sample4","sample5","sample6"),)
library(DESeq2)


countdata = read.table("",header = TRUE)
head(countdata)
colnames(countdata) = gsub("\\.[sb]am$","",colnames(countdata))
countdata = as.matrix(countdata)
head(countdata)
condition = factor(rep(1:2,each =))
library(DESeq2)
colData = data.frame(row.names = colnames(countdata))
dds = DESeqDataSetFromMatrix(countData = countdata,colData = colData,~condition)
dds
dds = DESeq(dds)
res= results(dds)
...
#Heatmaps
library(gplots)
new_merge_countfile=as.matrix(new_merge_countfile)
heatmap.2(new_merge_countfile,main = "Heatmap representation",margins = c(10,12),trace = "none",cexRow = 0.7,col = colorRampPalette(c("red","yellow","green","blue","black","brown")))

install.packages("devtools", dependencies = TRUE)
devtools::install_github("krlmlr/ulimit")
ulimit::memory_limit(2000)

trial=head(new_merge_countfile)
heatmapresult=heatmap.2(new_merge_countfile,main = "Heatmap representation",margins = c(10,12))
memory.limit(24000)
memory.limit()
